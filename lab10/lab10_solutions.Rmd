...
precipdf <- data.frame(precip, names(precip)) # Fill this in.
# Now merge `precipdf` with data frames `A` and `B`.
# You should obtain a data frame with 8 rows and 3 columns.
# Fill this in.
names(precipdf) <- c("precip", "city")
precipAB <- merge(vertAB, precipdf)
print(precipAB)
```
...
```{r}
# Fill this in.
precipABall <- merge(vertAB, precipdf, all = TRUE)
head(precipABall, n = 12)
```
...
```{r}
# Remove `eval = FALSE` or set `eval = TRUE` when you have completed this code chunk.
pop77df <- data.frame(State = rownames(pop77), pop77, row.names = NULL) # Fill this in.
head(pop77df)
```
...
# Fill this in.
pop14 <- pop14[, c(1, 8)]
names(pop14) = c("State", 2014)
pop14$State <- sub("\\.", "", pop14$State)
head(pop14, n = 12)
```
...
```{r}
# Fill this in.
pop77df$Population <- pop77df$Population * 1000
pop14$"2014" <- as.numeric(gsub(",", "", pop14$"2014"))
popmerge <- merge(pop14, pop77df)
head(popmerge)
```
